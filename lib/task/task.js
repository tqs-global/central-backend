...
const analyticsReporter = require('../external/odk-reporter').init(config.get('default.external.analytics'));
...
const task = {
  ...
  withContainer: (taskdef) => (...args) => {
    ...
    if (needsContainer) task._container = container.withDefaults({ db: slonikPool(config.get('default.database')), env, mail, task: true, analyticsReporter, s3, enketo });
    ...
    return Sentry.startSpan(
      ...
      async () => {
        try {
          return await taskdef(task._container)(...args);
        } catch (err) {
          Sentry.captureException(err);
          throw err;
        } finally {
          if (needsContainer) {
            const { db } = task._container;
