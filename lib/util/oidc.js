...
function getLogoutUrl(idTokenHint, returnTo) {
  // Build Keycloak logout URL with post_logout_redirect_uri and optional id_token_hint
  const domain = config.get('default.env.domain');
  const { issuerUrl } = oidcConfig;
  const logoutEndpoint = `${issuerUrl}/protocol/openid-connect/logout`;
  
  const postLogoutRedirectUri = encodeURIComponent(returnTo || `${domain}/login`);
  const clientId = encodeURIComponent(oidcConfig.clientId || '');
  
  let url = `${logoutEndpoint}?client_id=${clientId}&post_logout_redirect_uri=${postLogoutRedirectUri}`;
