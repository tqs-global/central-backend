...
function getLogoutUrl() {
  ...
  // IMPORTANT: Use the external Keycloak URL (browser-accessible), not the internal issuerUrl
  const domain = config.get('default.env.domain');
  const externalKeycloakUrl = 'https://keycloak.localhost:8180';
  const logoutEndpoint = `${externalKeycloakUrl}/realms/india-datasets/protocol/openid-connect/logout`;
  const postLogoutRedirectUri = encodeURIComponent(`${domain}/login`);
  const clientId = encodeURIComponent(oidcConfig.clientId || '');
  return `${logoutEndpoint}?client_id=${clientId}&post_logout_redirect_uri=${postLogoutRedirectUri}`;
}
