...
const setManagedEncryption = (project, passphrase, hint, auth) => ({ Forms, Keys, Projects }) => {
  ...
  return generateManagedKey(passphrase)
    .then((keys) => Forms.lockDefs() // lock!
      .then(() => Promise.all([
        Forms.getByProjectId(auth, project.id, Form.AnyVersion, Form.IncludeXml),
        Keys.create(new Key({
...
${extend|| sql`
  ...
  left outer join
    (select "projectId", count(datasets.id)::integer as datasets, max("lastEntByDataset") as "lastEntity" from datasets
      left outer join
        (select "datasetId", max("createdAt") as "lastEntByDataset" from entities
          where "deletedAt" is null
          group by "datasetId") as entity_stats
        on datasets.id=entity_stats."datasetId"
      where "publishedAt" is not null
      group by "projectId") as dataset_stats
...
const getByName = (name, options = QueryOptions.none) => ({ maybeOne }) =>
  _get(maybeOne, options.withCondition({ name }));

module.exports = {
  create, update, del,
  setManagedEncryption,
  getAllByAuth, getById, getByName
};
