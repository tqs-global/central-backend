  ...
  service.get('/oidc/callback', anonymousEndpoint.html(async (container, _, req, res) => {
    try {
      ...
      if (host.includes('kaveri') || host.includes('localhost')) {
        targetProject = 'Agriculture Surveys';
      ...
      if (targetProject) {
        ...
        // Assign user to project as Data Collector (has project.read + submission.create)
        const roleOption = await container.Roles.getBySystemName('formfill');
        console.log(`[OIDC] Role lookup formfill isEmpty: ${roleOption.isEmpty()}`);
        ...
        if (roleOption.isDefined()) {
          ...
          try {
            ...
            console.log(`[OIDC] ✅ Successfully assigned user actor ${user.actor?.id} to project ${project.id}`);
          } catch (e) {
            console.log(`[OIDC] ⚠️ Assignment error (may be duplicate): ${e.message}`);
          }
        } else {
          console.log(`[OIDC] ❌ Role 'app-user' not found!`);
        }
      }
