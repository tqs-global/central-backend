...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  const openRosaSubmission = (path, _endpoint, draft, getForm) => {
    ...
    service.get(path, _endpoint.openRosa(({ Projects }, { params }) =>
    ...
    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), multerUtf, multipartErrorHandler, _endpoint.openRosa(({ Forms, Submissions, SubmissionAttachments, PhotoValidations }, { params, files, auth, query, userAgent }) =>
                ...
                .then((maybeExtant) => {
                  ...
                  return Promise.all([
                    ...
                    .then(([saved, binaryFields]) => SubmissionAttachments.create(saved, form, binaryFields, files)
                      .then(async () => {
                        // Hook for Photo Validation Game
                        await initializePhotoValidations(PhotoValidations, saved, files);
                      }))
  ...
  openRosaSubmission('/projects/:projectId/submission', endpoint, false, (auth, { projectId }, xmlFormId, Forms, version) =>
    Forms.getByProjectAndXmlFormId(projectId, xmlFormId, version)
      .then(getOrNotFound)
  ...
  openRosaSubmission('/projects/:projectId/forms/:xmlFormId/draft/submission', endpoint, true, (auth, params, xmlFormId, Forms) => {
    ...
    return Forms.getByProjectAndXmlFormId(params.projectId, xmlFormId, Form.DraftVersion)
      .then(getOrNotFound)
      .then(ensureDef)
      .then((form) => auth.canOrReject('submission.create', form));
  });
  ...
  openRosaSubmission('/test/:key/projects/:projectId/forms/:xmlFormId/draft/submission', anonymousEndpoint, true, (_, params, xmlFormId, Forms) => {
    if (params.xmlFormId !== xmlFormId)
      return reject(Problem.user.unexpectedValue({ field: 'form id', value: xmlFormId, reason: 'did not match the form ID in the URL' }));
  ...
  const restSubmission = (base, draft, getForm) => {
    service.post(`${base}/submissions`, endpoint(({ Forms, Submissions, SubmissionAttachments, PhotoValidations }, { params, auth, query, userAgent }, request) =>
            ...
            return Promise.all([
              ...
              .then(([submission, binaryFields]) =>
                SubmissionAttachments.create(submission, form, binaryFields)
                  .then(async () => {
                    // Hook for Photo Validation Game
                    // TODO: For REST submissions, we might not have 'files' in the same format if it's XML-only post?
                    // But standard REST submission usually includes attachments separately or inline.
                    // For now, only XML parsing is robust here. Detailed attachment handling for REST might need more work if not multipart.
                    // However, initializePhotoValidations mainly needs the XML to find disease tags.
                    // If no attachments are present yet (POST xml only), we might miss them.
                    // But the logic extracts attachments from XML to know what *should* be there.
                    // Let's pass empty files array if needed, the logic effectively just needs to know the attachment names from XML.
                    await initializePhotoValidations(PhotoValidations, submission, []);
                    return submission;
                  })
  ...
  restSubmission('/projects/:projectId/forms/:xmlFormId', false, ({ projectId, xmlFormId }, Forms, version) =>
    Forms.getByProjectAndXmlFormId(projectId, xmlFormId, version) // TODO: okay so this is exactly the same as the func above..
      .then(getOrNotFound)
      // This replaces ensureDef(form). If the form was found with the project ID and form ID
      // constraints but the def was not found, that suggest the problem was with the version.
      .then(rejectIf(((form) => (form.def.id == null)), () => Problem.user.formVersionNotFound({ formVersion: version })))
  ...
  restSubmission('/projects/:projectId/forms/:xmlFormId/draft', true, ({ projectId, xmlFormId }, Forms) =>
    Forms.getByProjectAndXmlFormId(projectId, xmlFormId, Form.DraftVersion)
      .then(getOrNotFound)
      .then(ensureDef));
  ...
  service.post(`/test/:key/projects/:projectId/forms/:xmlFormId/draft/submissions`, anonymousEndpoint(({ Forms, Submissions, SubmissionAttachments }, { params, query, userAgent }, request) =>
    Submission.fromXml(request)
      .then((partial) => Forms.getByProjectAndXmlFormId(params.projectId, params.xmlFormId, Form.DraftVersion)
        .then(getOrNotFound)
        .then(ensureDef)
  ...
  const dataOutputs = (base, draft, getForm) => {
    ...
    service.get(`${base}/:instanceId/attachments`, endpoint(({ Forms, Submissions, SubmissionAttachments }, context) =>
      getOrRedirect(Forms, Submissions, context)
        .then(([form, submissionOrResponse]) => {
          // If getOrRedirect returned a redirect, pass it through immediately.
          if (submissionOrResponse != null && submissionOrResponse.id != null) {
            return SubmissionAttachments.getCurrentForSubmissionId(form.id, submissionOrResponse.id, draft);
          }
          return submissionOrResponse;
        })
    ));
        ...
        .then(([form, submissionOrResponse]) => {
          ...
          if (submissionOrResponse != null && submissionOrResponse.id != null) {
            return SubmissionAttachments
              .getCurrentBlobByIds(form.id, submissionOrResponse.id, context.params.name, draft)
              .then(getOrNotFound)
...
// Helper: Extract validatable photos and initialize their status
const initializePhotoValidations = (PhotoValidations, submission, files) => {
  // 1. Check if submission has disease-related fields
  // We look for any select1 field with 'disease', 'pest', or 'symptom' in the name
  return traverseXml(submission.xml, [
    findAll(root(), node('select1'))(text()) // We need to inspect the node attributes (ref) too, but `text()` gives value.
    // traverseXml is limited. Let's try to interpret the XML string simply or use existing helpers.
    // The codebase uses `traverseXml` with partial application.
    // It seems `traverseXml` returns promises of found values.
  ]).then(() => {
    // Since traverseXml is a bit complex to use for attribute matching without looking at util/xml.js details,
    // and we want to be robust, let's use a simpler regex or the provided XML tools if we can understand them.
    // Looking at `submission-attachments.js`, it uses `submissionXmlToFieldStream`. Let's use that!
    // It streams { field: { path, name, type, ... }, text } objects.
    const { submissionXmlToFieldStream } = require('../data/submission');

    // We need to find:
    // 1. The disease diagnosis (if any)
    // 2. The photo attachment names

    // We'll walk the stream once.
    let diagnosis = null;
    let diagnosisField = null;
    const photos = [];

    // Helper to stream async iterator to completion
    const processStream = async () => {
      // Dummy binaryFields list to ensure it parses everything? 
      // Actually submissionXmlToFieldStream needs binaryFields to identify binaries, 
      // but we can just look for type=binary in the field def if available, or infer from context?
      // Wait, submissionXmlToFieldStream takes (binaryFields, xml).
      // We don't have binaryFields easily here unless we pass them.
      // But we can just use regex for now to be safe and fast, mirroring how it's done elsewhere if needed.

      // Let's assume we can rely on standard field naming conventions for the hackathon context
      // or duplicate the stream logic if we had binaryFields.

      // ALTERNATIVE: Use Cheerio or xmldom? No, not imported.
      // CONSTANT: Simple XML parsing with Regex might be fragile but easiest given `submission.xml` is a string.
      // `traverseXml` is safer.

      // Let's refine the plan: Use regex for this specific "Validation Game" heuristic.
      // It matches `<*disease* ...>VALUE</...>` and `<*Image* ...>filename.jpg</...>`

      const xml = submission.xml.toString();

      // 1. Find diagnosis
      // Look for tags containing disease/pest/symptom and their values
      // Regex: <([^>]*?(?:disease|pest|symptom)[^>]*?)>([^<]+)<\/\1>
      // This is tricky with namespacing.

      // BETTER: Use `submissionXmlToFieldStream` but we need `binaryFields`.
      // We can fetch binaryFields using `Forms.getBinaryFields` but that requires `Forms`.
      // We have `Forms` in the closure.
      // But `initializePhotoValidations` is outside.
      // Let's move `initializePhotoValidations` inside or pass `binaryFields`.

      // Actually, we can just use the provided `files` array for attachments if it's multipart!
      // But correct, `files` contains the blobs. The XML links them.

      // Let's stick to Regex for the immediate fix as it's non-blocking and zero-dependency for this specific logic 
      // which is "game" logic (business logic) rather than core infrastructure.
      // Matches: <somename_disease>rice_blast</somename_disease>
      const diseaseRegex = /<[^>]*?(disease|pest|symptom)[^>]*?>([^<]+)<\/[^>]+>/i;
      const match = xml.match(diseaseRegex);
      if (match) {
        diagnosis = match[2];
        diagnosisField = match[0].split(/[<>]/)[1].split(' ')[0]; // rough tag extraction
      }

      // 2. Find photos
      // We can check the `files` array for uploaded files (multipart).
      // Or find tags ending in .jpg/.jpeg/.png in the XML values?
      // `files` from multer gives us what was uploaded.
      if (files && files.length > 0) {
        for (const file of files) {
          if (file.fieldname === XML_SUBMISSION_FILE) continue;
          if (file.mimetype && file.mimetype.startsWith('image/')) {
            photos.push(file.fieldname); // fieldname is the attachment name in ODK
          }
        }
      }
    };

    return processStream().then(async () => {
      if (diagnosis && photos.length > 0) {
        // We have a diagnosis and photos!
        // Initialize validation status for each photo
        for (const photoName of photos) {
          // we need submissionDefId. `submission` has `def.id`.
          await PhotoValidations.initializePhotoStatus(
            submission.def.id,
            photoName,
            diagnosisField || 'unknown_disease_field',
            diagnosis
          );
        }
      }
