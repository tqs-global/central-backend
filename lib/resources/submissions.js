...
module.exports = (service, endpoint, anonymousEndpoint) => {
  ...
  const openRosaSubmission = (path, _endpoint, draft, getForm) => {
    ...
    // Nonstandard REST; OpenRosa-specific API.
    service.post(path, multipart.any(), multerUtf, multipartErrorHandler, _endpoint.openRosa(({ Forms, Submissions, SubmissionAttachments }, { params, files, auth, query, userAgent }) =>
